...
SET @ProjectIDs = '365'
	...
	DECLARE @Projects TABLE (id INT)
	
	IF ISNULL(DataLength(@ProjectIDs), 0) = 0
		insert 	INTO @Projects (id)
		select id FROM dbo.ObjectsFinance WHERE del = 0 
	ELSE
		insert 	INTO @Projects (id)
		select id FROM dbo.ObjectsFinance WHERE del = 0 and id IN (select id from dbo.TextSplit(@ProjectIDs, ','))
	
	DELETE 
		FROM _DivisionRowLink
	SELECT *
		FROM _DivisionRowLink  drl
		LEFT JOIN _Divisions d on d.id = drl.Division_id
		LEFT JOIN _DivisionTypes dt on dt.id = d.DivisionType_id and NotAutoChange = 1
	WHERE Division_id IN (SELECT id from dbo._Divisions where del = 0 AND DivisionType_ID IN (50, 43, 42, 38, 39, 45, 44))
		AND object_id = @Object_ID AND Line_id IN (SELECT id from @Projects) and dt.id is null
...
*/
	...
	FROM
		 ...
		 LEFT JOIN _Divisions d ON d.Id = ISNULL(Dd.Division_ID, 1744)
	WHERE P.Del = 0 and P.name NOT LIKE '%мкр%' AND P.id IN (SELECT id from @Projects) AND NOT EXISTS 
		(SELECT 1 from _Divisions where Id IN (SELECT Division_id from _DivisionRowLink where Object_id = @Object_id AND Line_id = P.id) AND DivisionType_ID = d.DivisionType_ID)
...
*/
	...
	FROM
		 ...
		 JOIN dbo._Divisions d ON d.Id = ISNULL(Dd.Division_ID, 1744) AND D.DivisionType_ID = 43
	WHERE P.Del = 0 AND dd.Division_ID IS NOT NULL AND P.id IN (SELECT id from @Projects)
		 AND NOT EXISTS (SELECT 1 from _Divisions where Id IN (SELECT Division_id from _DivisionRowLink where Object_id = @Object_id AND Line_id = P.id) AND DivisionType_ID = d.DivisionType_ID)
...
*/
	...
	WHERE P.Del = 0 AND P.id IN (SELECT id from @Projects)
		 AND NOT EXISTS (SELECT 1 from _Divisions where Id IN (SELECT Division_id from _DivisionRowLink where Object_id = @Object_id AND Line_id = P.id) AND DivisionType_ID = d1.DivisionType_ID)
...
*/
	...
	FROM 
		 ...
		 JOIN @division d on prv.Date between ISNULL(d.BeginDate, 0) and isnull(d.EndDate, '99991212') AND isnull(d.BeginDate, d.EndDate) IS NOT null
			and p.ObjectFinanceType_id = ISNULL(d.Ident, nullif(p.ObjectFinanceType_id, 1))
		 JOIN dbo._Divisions d1 on d1.id = d.id
	WHERE P.Del = 0 AND P.id IN (SELECT id from @Projects)
		 AND NOT EXISTS (SELECT 1 from _Divisions where Id IN (SELECT Division_id from _DivisionRowLink where Object_id = @Object_id AND Line_id = P.id) AND DivisionType_ID = d1.DivisionType_ID)
