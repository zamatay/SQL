...
select cast(''  as xml).query('
<Ad>
	<Description>{sql:column("Description")}</Description>
</Ad>
') from (select top 1 aod.id, aod.Description 
				from AvitoObjectDescription aod 
					left join AvitoProjectDetail apd on apd.AvitoProject_ID = aod.id and apd.del = 0
				where aod.del = 0  and (aod.RoomCount = 1 or aod.RoomCount is null)
				order by 
					case 
						when aod.RoomCount = 1 then 1
						when aod.RoomCount is null then 2
					end
			) a
...
-- explicit example
	declare 
		@file_ident							varchar(15) = 'link_for_export',
		@image_ident						varchar(15) = 'Image',
		@video_ident						varchar(15) = 'Video',
		@RealtyObject_table_id				int = 899,
		@AvitoObjectDescription_table_id	int = 55799;	

select top 10
	1 [tag],
	null [parent],
	f.id [Ad!1!Id!Element],
	'Нет' [Ad!1!AllowEmail!Element],
	'Отдел продаж' [Ad!1!ManagerName!Element],
	'78003337111' [Ad!1!ContactPhone!Element],
	Description [Ad!1!Description!CDATA],
	Images.item [Ad!1!Images!Element],
	Video.item [Ad!1!VideoURL!Element],
	'Квартиры' [Ad!1!Category!Element],
	'Продам' [Ad!1!OperationType!Element],
	cast(f.Square * fmp.Price as numeric(10,0)) [Ad!1!Price!Element],
	case when Square > 35 then cast(f.RoomsCount as varchar) else 'Студия' end [Ad!1!Rooms!Element],
	cast(f.Square as numeric(5,2)) [Ad!1!Square!Element],
	cast(f.SquareKitchen as numeric(5,2)) [Ad!1!KitchenSpace!Element],
	cast(f.SquareLiving as numeric(5,2)) [Ad!1!LivingSpace!Element],
	Floor [Ad!1!Floor!Element],
	max(f.Floor) over(partition by f.FinanceObject_id) [Ad!1!Floors!Element],
	aht.Name [Ad!1!HouseType!Element],
	awt.Name [Ad!1!WallsType!Element],
	'Новостройка' [Ad!1!MarketType!Element],
	ap.AvitoProject_id [Ad!1!NewDevelopmentId!Element],
	'Застройщик' [Ad!1!PropertyRights!Element],
	'Чистовая' [Ad!1!Decoration!Element]

from r_FlatObjects f
outer apply (select top 1 aod.id, aod.Description 
				from AvitoObjectDescription aod 
					left join AvitoProjectDetail apd on apd.AvitoProject_ID = aod.id and apd.del = 0
				where aod.del = 0  and (aod.RoomCount = 1 or aod.RoomCount is null)
				order by 
		...
		outer apply ( select * from (
			SELECT top 1 FileName [url] from _Files where del = 1 and table_id=@RealtyObject_table_id and Description = @file_ident and line_id = f.RealtyObject_id
				UNION ALL
			SELECT FileName from _Files where del = 0 and table_id=@AvitoObjectDescription_table_id and line_id = apd.id and Description = @image_ident
			) [Image] for xml AUTO, type 
		) Images(item)
		outer apply ( 
			SELECT top 1 FileName from _Files [Video]  where del = 0 and table_id=@AvitoObjectDescription_table_id and line_id = apd.id and Description = @video_ident 
		) video(item)
		cross apply (select top 1 Price from r_FlatMinPrices fmp where fmp.Flat_ID = f.id order by Date desc) fmp
		join AvitoProject ap on ap.Project_id = f.FinanceObject_id
		left join AvitoHouseType aht on aht.id = ap.AvitoHouseType_id
		left join AvitoWallsType awt on awt.id = ap.AvitoWallsType_id
where f.id in (150646)
for xml explicit, root('Ads')